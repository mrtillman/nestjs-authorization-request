...
getTokenUseCaseMock.setup(useCase => useCase.execute()).returns(Promise.resolve(authResponse));
getCountersUseCaseMock.setup(useCase => useCase.execute()).returns(Promise.resolve(counters));
renewTokenUseCaseMock.setup(useCase => useCase.execute()).returns(Promise.resolve(authResponse));
cacheMock.setup(cache => cache.getValue(KEYS.ACCESS_TOKEN)).returns(null);
cacheMock.setup(cache => cache.setValue(KEYS.ACCESS_TOKEN, It.IsAny<AuthorizationResponse>())).returns(null);
...
describe("AppController", () => {
  ...
  describe("/renewtoken", () => {
    beforeEach(() => {
      ...
      responseMock.setup(response => response.json(It.IsAny<any>())).returns(null);
      responseMock.setup(response => response.redirect(It.IsAny<string>())).returns(null);
    })
    it("should get authorization response", async () => {
      await appController.renewToken(responseMock.object());

      responseMock.verify(response => response.json(It.IsAny<any>()), called(1));
    ...
    it("should redirect if no authorization response", async () => {
      ...
      renewTokenUseCaseMock.setup(useCase => useCase.execute()).returns(Promise.resolve(null));
      appController = new AppController(
        getTokenUseCaseMock.object(),
        getCountersUseCaseMock.object(),
        renewTokenUseCaseMock.object(),
      ...
      responseMock.verify(response => response.json(It.IsAny<any>()), called(0));
